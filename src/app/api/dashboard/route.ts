  ...
  try {
    ...
    try {
      ...
      if (pRows.length > 0) {
        ...
        const dealCounts = await raw`SELECT stage, count(*)::int as c FROM deals WHERE tenant_id = ${tid} GROUP BY stage`;
    ...
    // Top 5 by LTV (safe — just skip bad data)
    let top5: any[] = [];
    try {
      const topRows = await raw`
        ...
        FROM contacts WHERE tenant_id = ${tid}
          ...
          AND custom_fields->>'ltv' ~ '^[0-9.]+$'
        ORDER BY (custom_fields->>'ltv')::numeric DESC LIMIT 5
      `;
      top5 = topRows.map((r: any) => {
    ...
    // Revenue — safe with regex filter
    let totalRevenue = 0;
    try {
      const [rev] = await raw`
        SELECT COALESCE(SUM((custom_fields->>'ltv')::numeric), 0) as total
        FROM contacts WHERE tenant_id = ${tid}
          AND custom_fields->>'ltv' IS NOT NULL
          AND custom_fields->>'ltv' ~ '^[0-9.]+$'
      `;
    ...
    return ok({
      ...
      totalDeals: counts.total_deals,
      activeDeals: counts.total_deals,
      wonDeals: 0,
      ...
      revenue: {
        ...
        totalPurchases: 0,
        avgLTV: counts.total_contacts > 0 ? Math.round((totalRevenue / counts.total_contacts) * 100) / 100 : 0,
        avgOrder: 0,
